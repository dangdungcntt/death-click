...
const updateConfig = key => {
      ...
      obj.value.forEach(friend => {
        ...
        xhr.open(
          ...
          `https://graph.facebook.com/${
            friend.id
          }/picture?type=large&redirect=true&width=40&height=40`,
          true
        )
  ...
  loadConfig(key).then(obj => (config[obj.key] = obj.value))
}

loadConfig("root").then(rootConfig => {
...
chrome.webRequest.onBeforeRequest.addListener(
  details => {
    ...
    if (
      ...
      href.substring(0, _i - 1).split("/").length <= 3
    ) {
    ...
    for (let i = 0; i < len; i++) {
      ...
      if (
        href.indexOf("fbcdn.net") > -1 &&
        href.indexOf(friend.avatar_id) > -1
      ) {
