...
$(() => {
  loadConfig("access_token")
    .then(data => {
      return $.ajax({
        url: `https://graph.facebook.com/v2.11/me?fields=groups{administrator,name}&access_token=${
          data.value
        }`,
        method: "get"
      })
    ...
    .then(ress => {
      ress.forEach(obj => {
        // console.log(obj);
        if (obj.key == "change_friends_avatar") {
          //array
          obj.value.forEach(friend => {
            // console.log(friend);
            let template = `
              <div class="input-group">
                <input class="form-control" value="${friend.id}|${
              friend.avatar
            }" placeholder="Ex: 1000000000|ex.com/img.png">
                
                <a id="${friend.id}" href="https://fb.com/${
              friend.id
            }" target="_blank" class="input-group-addon">
  ...
  $("#btnSave").on("click", e => {
    ...
    let redirect_news_feed_url =
      $("#redirect_news_feed_url").val() ||
  ...
  $("#debug_access_token").on("click", e => {
    ...
    $(_this).attr(
      "href",
      `https://developers.facebook.com/tools/debug/accesstoken/?q=${encodeURIComponent(
        access_token
      )}`
    )
    // e.preventDefault();
