...
const getUserIdFromLink = (link, cb) => {
    ...
    if (existId > -1) {
        ...
        loadConfig('access_token').then(access_token => {
            var str = link.split('/').slice(-1)[0]
...
const removeUser = (link, banUser = 0) => {
    loadConfig('target_group').then(group => {
        getUserIdFromLink(link, (id) => {
    ...
    if (message.functiontoInvoke) {
        switch (message.functiontoInvoke) {
            ...
            case 'getID':
                ...
                break
            case 'removeUser':
                removeUser(message.link);
                break
            case 'removeAndBanUser':
                removeUser(message.link, 1);
                break
        }
    }
})
...
if (location.href.indexOf('youtube.com') > -1) {
    ...
    var target = document.querySelector('ytd-app'),
        ...
        observer = new MutationObserver(function(mutation) {
          console.log(target.getAttribute('is-watch-page'));
            if (target.hasAttribute('is-watch-page') && location.href.indexOf('youtube.com/watch?v=') > -1 && !document.querySelector('#download-video-death-click')) {

                var a = document.createElement('a');
                a.id = 'download-video-death-click';
                a.onclick = function() {
                    var newtab = window.open(location.href.replace('www.youtube', 'outube').replace('https', 'http'), '_blank');
                    newtab.focus();
                }
                a.style.cssText = "    text-decoration: none;font-family: 'Roboto','Noto',sans-serif;padding: 6px 12px;background: #ff0000;border-radius: 2px;text-transform: uppercase;color: #fff;font-weight: 500;display: inline-block;margin-right: 10px;cursor: pointer;"
                a.target = "_blank";
                a.innerText = "Download";
                var itv = setInterval(function() {

                    if (document.querySelector('#download-video-death-click')) {
                      clearInterval(itv);
                      return;
                    }
