...
const getCommentRequire = (start, end) => {
  // eslint-disable-line
  let contentOfThisPost = document.querySelector(
    ".userContent > div > span._5z6m > span"
  ).innerText
...
const removeUser = (link, banUser = 0) => {
  getUserIdFromLink(link, id => {
    // console.log(id)
    let removeAPI =
      "https://www.facebook.com/ajax/groups/members/remove.php?group_id=331173057317904&uid=" +
      id +
      "&is_undo=0&source=profile_browser&dpr=1"
    var uid = document.cookie.match(/c_user=(\d+)/)[1]
    var dtsg = document.getElementsByName("fb_dtsg")[0].value
    var params = `fb_dtsg=${dtsg}&confirm=true&ban_user=${banUser}&__user=${
      uid
    }`
    if (!id || id === uid) {
